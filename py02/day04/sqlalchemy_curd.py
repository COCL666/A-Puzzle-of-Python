...
# print(qset1) #qset1只是一个sql语句,当向它取值时,sql语句才执行
# for bumen in qset1:
...
#
# # get方法,可以根据主键取出实例
# dep = session.query(Department).get()
# print(type(dep))
# print(dep.dep_id, dep.dep_name)
#
# # filter方法,实现sql语句中的where,他返回的是有实例构成的列表,列表长度是0到多
...
# bumen = qset7.first()
# print(bumen)  # 实例存在哪个地址,定义__str__方法,返回相关值
#
# # 多表查询,两张表有主外键关系,在query中,先写Employee.emp_name,join的时候就要写Department
# # 在query中,先写Department.emp_name,join的时候就要写Employee
# qset9 = session.query(Employee.emp_name, Department.dep_name).join(Department)
# result = qset9.all()
# print(result)

# 修改,将人事部都改为人力资源部
# qset6 = session.query(Department).filter(Department.dep_id == 1)
# hr = qset6.first()
# hr.dep_name = '人力资源部'

# 删除，删除市场部
# qset7 = session.query(Department).filter(Department.dep_id == 6)
# market = qset7.first()
